...
class Display extends Component {
  state = {randomNumber: 0, tab: 'FRUIT'}

  changeImageSelect = e => {
    const {randomNumber} = this.state

    if (e.target.id === imagesList[randomNumber].id) {
      this.setState(prevState => ({
        ...prevState,

        randomNumber: Math.floor(Math.random() * (imagesList.length - 1)),

        tab: imagesList.find(image => image.id === e.target.id).category,
      }))
      console.log('Correct')
      //   this.props.setScore
  ...
  render() {
    ...
    //   const {imagesList, RandomNum} = props
    ...
    return (
      <>
        ...
        <Displayer
          ...
          tabsList={tabsList}
          lowerImageSelect={this.changeImageSelect}
        />
      </>
    )
  }
